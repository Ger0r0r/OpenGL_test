...
int main()
{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		// render container
		ourShader.use();
		ourShader.setFloat("proc", proc_v);
		...
		if (timeValue - lastTime >= 1.0 ) { // If last prinf() was more than 1 sec ago
			...
			std::cout << nbFrames << "\t" << 1000.0/(float)nbFrames << " ms\t" <<  std::endl;
			std::cout << "value " << proc_v << "\n" <<  std::endl;
			nbFrames = 0;
			lastTime += 1.0;
		}
	}
...
void processInput(GLFWwindow *window)
{
	...
	{
		proc_v += 0.001f; // change this value accordingly (might be too slow or too fast based on system hardware)
		if(proc_v >= 1.0f)
			proc_v = 1.0f;
	}
